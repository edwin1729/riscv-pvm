...
METRICS_ARGS=()
NATIVE=""
REVM_SANDBOX_PARAMS=("--input" "kernels/revm/target/riscv64gc-unknown-linux-musl/release/revm-kernel")
...
while getopts "i:t:m:spnj:hu" OPTION; do
  ...
  p)
    ...
    ;;
  n)
    NATIVE=$(make --silent -C kernels/revm print-native-target | grep -wv make)
    ;;
  j)
    case "$OPTARG" in
    i*)
...
make -C kernels/revm inbox-bench &> /dev/null
...
kernels/revm/inbox-bench generate --inbox-file "$INBOX_FILE" --transfers "$TX"
...
build_revm_riscv() {
  if [ "$STATIC_INBOX" = "y" ]; then
    INBOX_FILE="$INBOX_FILE" make -C kernels/revm build-kernel-static CONFIG_FLAGS="$CONFIG_FLAGS" &> /dev/null
    RUN_INBOX="$DATA_DIR"/empty.json
    echo "[]" > "$RUN_INBOX"
  else
    make -C kernels/revm build-kernel CONFIG_FLAGS="$CONFIG_FLAGS"&> /dev/null
  fi
}
...
##########
build_revm_native() {
  INBOX_FILE=$INBOX_FILE make -C kernels/revm build-kernel-native CONFIG_FLAGS="$CONFIG_FLAGS"&> /dev/null
}

run_revm_native() {
  LOG="$DATA_DIR/log.$1.log"
  $PROFILING_WRAPPER kernels/revm/target/"$NATIVE"/release/revm-kernel \
...
collect() {
  echo -e "\033[1m"
  kernels/revm/inbox-bench results --inbox-file "$INBOX_FILE" "${log_file_args[@]}" --expected-transfers "$TX"
  echo -e "\033[0m"
}
